...
const AIDoctorChat = () => {
  ...
  return (
    <div className="flex flex-col bg-white max-h-[70vh] w-full">
      {/* Chat Messages - Scrollable Area */}
      <div
        ref={messagesContainerRef}
        className="flex-1 overflow-y-auto px-4 py-6 space-y-5 bg-slate-50/50"
        style={{
      ...
      >
        {messages.map((message) => (
          ...
          >
            <div
              className={`flex items-end max-w-[88%] gap-2 ${message.sender === "user" ? "flex-row-reverse" : "flex-row"}`}
            >
              {/* Avatar */}
              <div
                className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center shadow-sm z-10 ${
                  message.sender === "user"
                    ? "bg-gradient-to-br from-[#0067A1] to-[#0a4440] text-white"
                    : "bg-gradient-to-br from-emerald-100 to-teal-100 border border-emerald-200 text-[#0067A1]"
                }`}
              >
                {message.sender === "user" ? (
              ...
              {/* Message Bubble */}
              <div
                className={`px-4 py-3 transform transition-all ${
                  message.sender === "user"
                    ? "bg-gradient-to-r from-[#0067A1] to-[#0080C6] text-white rounded-2xl rounded-br-sm shadow-md"
                    : "bg-white text-gray-800 border border-gray-100/80 rounded-2xl rounded-bl-sm shadow-sm"
                }`}
              >
        ...
        {isLoading && (
          <div className="flex justify-start animate-fadeIn">
            <div className="flex items-end gap-2">
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-emerald-100 to-teal-100 border border-emerald-200 flex items-center justify-center text-[#0067A1] shadow-sm z-10">
                <FaUserMd className="w-4 h-4" />
              </div>
              <div className="bg-white rounded-2xl rounded-bl-sm px-4 py-3 shadow-sm border border-gray-100/80">
                <div className="flex items-center space-x-2">
                  <FaSpinner className="w-4 h-4 text-[#0067A1] animate-spin" />
      ...
      <div className="bg-white/80 backdrop-blur-md px-4 py-3 border-t border-gray-100 shadow-[0_-10px_30px_rgba(0,0,0,0.02)] flex-shrink-0 relative z-20">
        <form onSubmit={handleSubmit} className="flex items-center gap-2">
          <input
            ...
            disabled={isLoading}
            className="flex-1 bg-gray-50/80 border border-gray-200 rounded-full px-5 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#0067A1]/30 focus:border-[#0067A1] transition-all text-gray-900 placeholder-gray-400 disabled:opacity-50 shadow-sm"
          />
          <button
            type="submit"
            disabled={isLoading || !inputValue.trim()}
            className="bg-gradient-to-r from-[#0067A1] to-[#0080C6] hover:shadow-lg text-white p-3 rounded-full transition-all disabled:opacity-50 disabled:cursor-not-allowed transform hover:-translate-y-0.5 active:translate-y-0"
          >
            <FaPaperPlane className="w-4 h-4 ml-0.5 mt-0.5" />
          </button>
        </form>
        <div className="mt-3 overflow-x-auto pb-1 scrollbar-hide">
          <div className="flex gap-2 min-w-max">
            <button
              ...
              onClick={() =>
                !isLoading &&
                setInputValue("I have chest pain and shortness of breath.")
              }
              className="text-[11px] px-3.5 py-1.5 rounded-full bg-emerald-50/50 hover:bg-emerald-50 border border-emerald-100 text-[#0067A1] font-medium disabled:opacity-50 transition-all hover:shadow-sm"
            >
            ...
            <button
              ...
              onClick={() =>
                !isLoading &&
                setInputValue("I have fever, cough, and body pain for 3 days.")
              }
              className="text-[11px] px-3.5 py-1.5 rounded-full bg-emerald-50/50 hover:bg-emerald-50 border border-emerald-100 text-[#0067A1] font-medium disabled:opacity-50 transition-all hover:shadow-sm"
            >
            ...
            <button
              ...
              onClick={() =>
                !isLoading &&
                setInputValue("I feel anxious and unable to sleep properly.")
              }
              className="text-[11px] px-3.5 py-1.5 rounded-full bg-emerald-50/50 hover:bg-emerald-50 border border-emerald-100 text-[#0067A1] font-medium disabled:opacity-50 transition-all hover:shadow-sm"
            >
        ...
        <p className="text-[10px] text-gray-400/80 text-center mt-2 font-medium tracking-wide">
          For informational purposes only · Always consult a verified doctor
        </p>
      </div>
      {analysis && (
