...
import {
  ...
  CheckCircle2, AlertCircle, Loader2, Shield, ChevronDown,
  Stethoscope, Home, Users, Activity, HandHeart
} from "lucide-react";
import Link from "next/link";

const CARE_TYPES = [
  { id: "Home Nursing Support", label: "Home Nursing Support", icon: Home, desc: "General nursing care at home" },
  { id: "Elder Care Assistance", label: "Elder Care Assistance", icon: Users, desc: "Dedicated care for elderly family members" },
...
const DURATIONS = [
  ...
  { value: "short_term", label: "Short-term (few days)" },
  { value: "long_term", label: "Long-term (weeks/months)" },
];

export default function NursingCareRequestPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name || !phone || !city || !duration || careTypes.length === 0) {
      setError("Please fill in all required fields and select at least one care type.");
      return;
    }
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/nursing/request", {
        method: "POST",
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#e6f2f1] via-white to-[#d5eeec] flex items-center justify-center p-4">
        ...
        >
          ...
          <p className="text-gray-600 mb-4">
            Your nursing care request has been received. Our team will contact you shortly.
          </p>
          ...
          <p className="text-xs text-gray-400 mb-6">
            Service delivery is handled directly by independent service providers.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link
              href="/website"
              className="inline-block px-6 py-3 bg-gray-100 text-gray-700 rounded-xl font-medium hover:bg-gray-200 transition-colors"
            >
              Back to Home
            </Link>
          </div>
      ...
      <div className="bg-gradient-to-br from-[#0067A1] to-[#0080C6] text-white py-12 px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h1 className="text-3xl lg:text-4xl font-bold mb-3">
            Request Nursing / Home Care
          </h1>
          <p className="text-white/80 text-sm lg:text-base max-w-xl mx-auto">
            MediConnect coordinates non-emergency service requests. Fill the form below and our team will connect with you.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 -mt-4">
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <p className="text-sm text-amber-800">
            <strong>Disclaimer:</strong> Service delivery is handled directly by independent service providers. 
            MediConnect facilitates service requests and connects users with independent providers. 
            MediConnect does not directly provide nursing or home care services.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-3xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {CARE_TYPES.map((ct) => {
              const Icon = ct.icon;
              const selected = careTypes.includes(ct.id);
              return (
                <button
                  key={ct.id}
                  type="button"
                  onClick={() => toggleCareType(ct.id)}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
            <p className="text-xs text-amber-700">
              <strong>⚠ Warning:</strong> Please do not share medical reports, prescriptions, or sensitive documents here.
            </p>
          </div>
          <textarea
            value={notes}
            ...
            maxLength={1000}
            rows={4}
            placeholder="Any additional details about your care requirement..."
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <div>
                <p className="text-sm font-medium text-gray-800 leading-relaxed">
                  ...
                  for coordinating nursing and home care services with verified service partners.
                </p>
                <p className="text-xs text-gray-400 mt-1">Data handling as per our{" "}
                  <Link href="/website/nursing-care/privacy" className="text-[#0067A1] underline hover:text-[#0080C6]">Privacy Policy</Link>
                </p>
              </div>
            </label>
            ...
            >
              ...
              <div>
                <p className="text-sm font-medium text-gray-800 leading-relaxed">
                  I agree to receive calls, SMS, or WhatsApp messages regarding my service request.
                </p>
                <p className="text-xs text-gray-400 mt-1">Review our{" "}
                  <Link href="/website/nursing-care/terms" className="text-[#0067A1] underline hover:text-[#0080C6]">Terms of Use</Link>
                </p>
              </div>
            </label>
          </div>
        ...
        <div className="flex items-center justify-center gap-6 text-sm">
          <Link href="/website/nursing-care/privacy" className="text-[#0067A1] hover:underline">
            Privacy Policy
          </Link>
          <span className="text-gray-300">|</span>
          <Link href="/website/nursing-care/terms" className="text-[#0067A1] hover:underline">
            Terms of Use
          </Link>
        </div>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center">
          <p className="text-xs text-gray-500 leading-relaxed">
            MediConnect facilitates service requests and connects users with independent service providers. 
            MediConnect does not directly provide nursing or home care services.
          </p>
        </div>
      </form>
