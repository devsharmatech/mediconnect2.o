...
import {
    FaFlask, FaSearch, FaMapMarkerAlt, FaStar, FaHome, FaClock,
    FaArrowRight, FaTimes, FaPhone, FaVial, FaUserMd, FaTruck,
    FaStarHalfAlt, FaRegStar
} from "react-icons/fa";
import { motion } from "framer-motion";
import toast from "react-hot-toast";

export default function BrowseLabsPage() {
    ...
    const fetchLabs = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                toast.error(data.message || "Failed to load labs");
            }
        } catch {
    ...
    return (
        <div className="min-h-screen">
            {/* Hero Header */}
            <div className="relative overflow-hidden bg-gradient-to-br from-[#0067A1] via-[#0080C6] to-[#127a72] rounded-3xl px-6 sm:px-10 pt-6 pb-8 mb-6 shadow-xl shadow-[#0067A1]/15">
                ...
                <div className="absolute top-1/2 right-16 w-20 h-20 bg-white/5 rounded-full hidden sm:block" />
                <div className="relative">
                    <div className="flex items-center gap-4 mb-3">
                        ...
                        <div>
                            <h1 className="text-xl sm:text-3xl font-extrabold text-white tracking-tight">Book Lab Tests</h1>
                            <p className="text-white/60 text-xs sm:text-sm mt-0.5">Browse accredited labs, compare prices & book tests online</p>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSearch} className="mt-5 flex gap-3">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                className="w-full pl-11 pr-4 py-3 rounded-xl bg-white text-gray-900 placeholder-gray-400 focus:ring-4 focus:ring-white/30 focus:outline-none text-sm shadow-xl"
                            />
                        </div>
                        <button type="submit" className="px-5 py-3 bg-white/20 backdrop-blur-sm text-white rounded-xl font-semibold hover:bg-white/30 transition-colors border border-white/20 cursor-pointer text-sm hidden sm:block">
                            Search
                        </button>
                    </form>
                    ...
                    <div className="mt-5 flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-4">
                        <button
                            onClick={() => { setHomeCollection(!homeCollection); setPage(1); }}
                            className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold transition-all border cursor-pointer ${homeCollection
                        ...
                        >
                            ...
                            Home Collection
                            {homeCollection && <FaTimes className="w-2.5 h-2.5 ml-1" />}
                        </button>

                        {!loading && labs.length > 0 && (
                            <div className="flex items-center gap-4">
                                <div className="text-right">
                                    <p className="text-lg leading-tight font-bold text-white">{labs.length}</p>
                                    <p className="text-white/40 text-[10px] uppercase tracking-wider">Labs Found</p>
                                </div>
                                <div className="w-px h-6 bg-white/10" />
                                <div className="text-left">
                                    <p className="text-lg leading-tight font-bold text-emerald-300">{labs.filter(l => l.accepts_home_collection).length}</p>
                                    <p className="text-white/40 text-[10px] uppercase tracking-wider">Home Collection</p>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-3xl border border-gray-100 p-16 text-center shadow-sm">
                    ...
                    <p className="text-gray-500 max-w-md mx-auto">
                        {search ? "Try a different search term or location." : "There are currently no labs available matching your criteria."}
                    </p>
                    {search && (
                        <button
                            onClick={() => { setSearch(""); setHomeCollection(false); setPage(1); fetchLabs(); }}
                            className="mt-6 font-bold text-[#0067A1] hover:underline"
                        >
                            Clear All Filters
                        </button>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {labs.map((lab, idx) => (
                            <motion.div
                                ...
                                onClick={() => router.push(`/website/dashboard/lab-booking/${lab.id}`)}
                                className="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-xl hover:shadow-[#0067A1]/5 hover:border-[#0067A1]/30 transition-all duration-300 cursor-pointer group flex flex-col h-full relative"
                            >
                                ...
                                <div className="px-6 py-4 bg-gray-50/80 border-t border-gray-100 flex items-center justify-between group-hover:bg-[#0067A1]/5 transition-colors">
                                    <span className="text-sm font-bold text-[#0067A1]">Book Tests</span>
