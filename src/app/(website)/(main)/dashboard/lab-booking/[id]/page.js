...
export default function LabTestsPage({ params }) {
    ...
    return (
        <div className="min-h-screen pb-28">
            {/* Header */}
            <div className="relative overflow-hidden bg-gradient-to-br from-[#0067A1] via-[#0080C6] to-[#127a72] rounded-3xl px-6 sm:px-10 pt-7 pb-8 mb-6 shadow-xl shadow-[#0067A1]/15">
                                    ...
                                    <motion.div key={test.id}
                                        ...
                                        <div className="p-5">
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="flex-1 min-w-0">
                                                    {test.test_code && (
                                                        <span className="text-[10px] font-mono text-gray-400 bg-gray-50 px-2 py-0.5 rounded inline-block mb-1">{test.test_code}</span>
                                                    )}
                                                    <h3 className="text-sm font-bold text-gray-900 leading-snug">{test.test_name}</h3>
                                                    {test.category?.name && (
                                                        <span className="text-[11px] text-[#0067A1] font-medium mt-1 block">{test.category.name}</span>
                                                    )}
                                                </div>
                        ...
                        <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
                            <button onClick={() => setShowCart(!showCart)}
                                ...
                                <div className="relative">
                                    ...
                                    <span className="absolute -top-2 -right-2 w-5 h-5 bg-[#0067A1] text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                                        {cart.length}
                                    </span>
                                </div>
                        ...
                        <AnimatePresence>
                            {showCart && (
                                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                                    ...
                                    <div className="max-w-5xl mx-auto space-y-2 max-h-48 overflow-y-auto">
                                        {cart.map(item => (
